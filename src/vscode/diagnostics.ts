...
export function activateDiagnostics(
  ...
  const runTokenByUri = new Map<string, number>();
  const timerByUri = new Map<string, NodeJS.Timeout>();

  const run = async (doc: vscode.TextDocument, token: number) => {
    ...
    const includeResults = analyzeIncludes(
      text,
      doc.fileName,
      includePaths,
      buildIncludeErrorMessage,
    );
    ...
    if (runTokenByUri.get(doc.uri.toString()) !== token) return;
  ...
  const scheduleRun = (doc: vscode.TextDocument, delayMs = 250) => {
    if (!isPawnFile(doc.fileName)) return;
  ...
  const runActive = () => {
    ...
    if (ed) scheduleRun(ed.document, 50);
  };

  context.subscriptions.push(
    vscode.workspace.onDidOpenTextDocument(doc => scheduleRun(doc, 50)),
    vscode.workspace.onDidSaveTextDocument(doc => scheduleRun(doc, 0)),
    vscode.workspace.onDidChangeTextDocument(e => scheduleRun(e.document)),
