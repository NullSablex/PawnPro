...
import { getFileSymbols, getIncludedFiles, SigInfo } from '../core/fileCache.js';
import { PawnProConfigManager } from '../core/config.js';
import { getWorkspaceRoot } from './configBridge.js';

export type { SigInfo } from '../core/fileCache.js';

// Document-level cache (keyed by version, not mtime)
type DocIntelliCache = {
  version: number;
  sigs: Map<string, SigInfo>;
  macros: Set<string>;
};
const docIntelliCache = new Map<string, DocIntelliCache>();
...
function normalizeExternalBounds(sig: SigInfo): SigInfo {
  if (sig.maxArgs === null) return sig;

  const hasFlexibleMarkers =
    sig.signature.includes('=') ||
    sig.signature.includes('...') ||
    /\{[^}]*,[^}]*\}/.test(sig.signature);

  if (!hasFlexibleMarkers) return sig;
...
export async function buildIntelliData(
  document: vscode.TextDocument,
  config: PawnProConfigManager,
): Promise<{ sigs: Map<string, SigInfo>; macros: Set<string> }> {
  ...
  const sigs = new Map<string, SigInfo>(local.sigs);
  const macros = new Set<string>(local.macros);
  ...
  try {
    ...
    for (const fp of includedFiles) {
      if (fp === document.fileName) continue;
      const fromFile = await getFileSymbols(fp);
      for (const [name, sig] of fromFile.sigs) {
        if (!sigs.has(name)) sigs.set(name, normalizeExternalBounds(sig));
      }
      for (const m of fromFile.macros) macros.add(m);
    }
  } catch {
  ...
  if (docIntelliCache.size > 20) {
    ...
    if (first) docIntelliCache.delete(first);
  }

  return { sigs, macros };
}

export function signatureToDetail(sig: SigInfo): string {
