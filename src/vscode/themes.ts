...
function getThemeKind(): ThemeKind {
  ...
  if (k === vscode.ColorThemeKind.Dark || k === vscode.ColorThemeKind.HighContrast) return 'dark';
  if (k === vscode.ColorThemeKind.HighContrastLight) return 'light';
  return 'light';
}

function themedKey(name: string): string {
...
export function registerSyntaxSchemeCommands(
  ...
  context.subscriptions.push(
    vscode.window.onDidChangeActiveColorTheme(() => {
      ...
      if (!(cfg.syntax.applyOnStartup && cfg.syntax.scheme === 'auto')) return;
...
export async function applySchemeOnActivate(
  context: vscode.ExtensionContext,
  config: PawnProConfigManager,
) {
  const cfg = config.getAll();
