...
export async function buildIncludePaths(): Promise<string[]> {
  const cfg = vscode.workspace.getConfiguration();
  const ws = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath || '';

  const fromSettings = (cfg.get<string[]>('pawnpro.includePaths') || []).map(expandWS);
  const fromArgs = getIncludePathsFromArgs(cfg.get<string[]>('pawnpro.compiler.args') || []);
...
export function activateIncludeDiagnostics(context: vscode.ExtensionContext) {
    ...
    while ((m = INCLUDE_RX_GLOBAL.exec(text))) {
      ...
      if (!resolved) {
        const d = new vscode.Diagnostic(
          range,
          localize('include.notFound', 'Include não encontrado: {0}', token),
  ...
  const runActive = () => {
    ...
    if (ed) void run(ed.document);
  };

  context.subscriptions.push(
    vscode.workspace.onDidOpenTextDocument(doc => void run(doc)),
    vscode.workspace.onDidSaveTextDocument(doc => void run(doc)),
    vscode.workspace.onDidChangeTextDocument(e => void run(e.document)),
