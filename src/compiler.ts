...
import { detectSupportedFlags, computeMinimalArgs, Supported } from './flags.js';
...
function sanitizeUserArgs(baseArgs: string[], s: Supported, channel: vscode.OutputChannel): string[] {
  ...
  for (const a of args) {
    ...
    if (!key) { kept.push(a); continue; } // não parece opção (-w etc.), mantém
    const ok = (key.length === 1 ? s.single.has(key) : s.multi.has(key));
    if (ok) kept.push(a);
    else channel.appendLine(`[PawnPro] Removendo flag não suportada para este pawncc: ${a}`);
  }
  return kept;
}
...
export function registerCompileCommand(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pawnpro.detectCompiler', async () => {
      try {
        ...
        const exe = detectPawncc(
          cfg.get<string>('pawnpro.compiler.path') || '',
          cfg.get<boolean>('pawnpro.compiler.autoDetect') ?? true
        );
  ...
  const cmd = vscode.commands.registerCommand('pawnpro.compileCurrent', async () => {
    ...
    let rawArgs = (cfg.get<string[]>('pawnpro.compiler.args') || []).slice();
    if (rawArgs.length === 0) {
    ...
    const includePaths = (cfg.get<string[]>('pawnpro.includePaths') || [])
    ...
    // 6) log
    if (cfg.get<boolean>('pawnpro.build.showCommand')) {
    ...
    // 7) spawn
    const encoding = cfg.get<string>('pawnpro.output.encoding') || 'windows1252';
    const proc = spawn(exe, args, { cwd: fileDir, shell: false });

    proc.stdout.on('data', (d) => channel.append(iconv.decode(d as Buffer, encoding)));
    proc.stderr.on('data', (d) => channel.append(iconv.decode(d as Buffer, encoding)));

    proc.on('error', (err) =>
      vscode.window.showErrorMessage(localize('build.startError', 'Falha ao iniciar pawncc: {0}', err.message))
    );
    proc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        vscode.window.showErrorMessage(
          localize('build.failed', 'Compilação falhou. code={0} signal={1}', code ?? 'null', signal ?? 'none')
        );
      }
    });
  });
