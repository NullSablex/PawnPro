...
export function activate(context: vscode.ExtensionContext) {
  try {
    ...
    const getIncludePaths = async (): Promise<string[]> => {
      if (cachedIncludePaths) return cachedIncludePaths;
      const ws = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath || '';
      const fromSettings =
        (vscode.workspace.getConfiguration().get<string[]>('pawnpro.includePaths') || [])
    ...
    const focusContainerIfNeeded = () => {
      const separate = vscode.workspace
        .getConfiguration()
        .get<boolean>('pawnpro.ui.separateContainer', false);
