 ...
 */
export function analyzeSemantics(text: string): DiagnosticData[] {
  ...
  for (let i = 0; i < cleanLines.length; i++) {
    ...
    if (mNative && !rxForward.test(line)) {
      ...
      if (trail === '{') {
        ...
        pushDiagnostic(diagnostics, lineOffsets, i, col >= 0 ? col : 0, name.length,
          `Função native "${name}" não pode ter corpo.`, 'error');
      }
      blockDepth = updateBraceDepth(line, blockDepth);
      continue;
    }
    ...
    if (mForward) {
      ...
      if (trail === '{') {
        ...
        pushDiagnostic(diagnostics, lineOffsets, i, col >= 0 ? col : 0, name.length,
          `Declaração forward "${name}" não pode ter corpo.`, 'error');
      }
      blockDepth = updateBraceDepth(line, blockDepth);
      continue;
    }
    ...
    if (mPS) {
      ...
      if (trail === ';') {
        ...
        pushDiagnostic(diagnostics, lineOffsets, i, col >= 0 ? col : 0, name.length,
          `Declaração ${kind} "${name}" sem corpo. Use "forward" para protótipos.`, 'warning');
      }
    }
