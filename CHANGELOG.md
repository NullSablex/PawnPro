...
### Alterado
- **Painel de comandos do servidor revisto** — o botão "Enviar" vira um ícone, e **Recentes** e **Favoritos** passam a dividir o mesmo espaço em duas abas, com a contagem de itens em cada uma; antes eram duas listas empilhadas, que espremiam o painel. A estrela dos favoritos deixa de ser um emoji e passa a acompanhar o tema — contornada quando o comando não é favorito, cheia e destacada quando é. O layout encolhe junto com o painel (rótulos, ícones e a dica reduzem por `clamp`, e a contagem sai quando o espaço aperta), em vez de quebrar. O campo de comando agora diz como escrever: **sem barra** (`gmx`, `kick 0`) — o prefixo `rcon` é removido automaticamente quando presente
