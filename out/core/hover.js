...
function parseMacroLocalFunctionsByLines(lines, defs) {
...
function prettyPathFromWorkspace(resolved, workspaceRoot) {
    if (!workspaceRoot)
...
export async function computeHover(params) {
    ...
    if (posInAnyString(character, lineText, spans))
        return null;
    ...
    for (const ident of tryIdentsInOrder) {
        ...
        if (defFn) {
            ...
            if (body)
                sections.push({ kind: 'code', content: body, language: 'pawn' });
            return { sections };
        }
