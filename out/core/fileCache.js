...
const identsCache = new Map();
const symbolsCache = new Map();
const funcsCache = new Map();
const apiCache = new Map();
const includesCache = new Map();
const apiIndexCache = new Map();
...
export async function getFileIdents(filePath) {
    const mtime = await getMtime(filePath);
    if (mtime === null)
        return new Set();
...
export async function getFileSymbols(filePath) {
    const mtime = await getMtime(filePath);
    if (mtime === null)
        return { sigs: new Map(), macros: new Set() };
...
function parseTextFunctions(text) {
    const lines = text.split(/\r?\n/);
    const out = [];
...
function chooseBestFuncs(funcs) {
...
export async function getFileFunctions(filePath) {
    const mtime = await getMtime(filePath);
    if (mtime === null)
        return [];
...
function parseApiFile(file, text) {
    const map = new Map();
...
export async function getApiFileFunctions(filePath) {
    const mtime = await getMtime(filePath);
    if (mtime === null)
        return new Map();
...
async function* walkIncludePaths(dir) {
    let ents;
    try {
...
export async function getApiIndex(includePaths) {
...
export async function getIncludedFiles(rootFilePath, includePaths, maxDepth = 5, maxFiles = 100) {
...
export function setDocumentText(filePath, text, version) {
...
/* ─── Pre-warming ──────────────────────────────────────────────── */
export async function prewarmCache(rootFiles, includePaths) {
