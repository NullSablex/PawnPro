...
function getCfg() {
    ...
    return {
        exe: substWorkspace(cfg.get('pawnpro.server.path') || ''),
        cwd: substWorkspace(cfg.get('pawnpro.server.cwd') || '${workspaceFolder}'),
        args: (cfg.get('pawnpro.server.args') || []).map(substWorkspace),
        clearOnStart: cfg.get('pawnpro.server.clearOnStart') ?? true,
        logPath: substWorkspace(cfg.get('pawnpro.server.logPath') || ''),
        logEncoding: (cfg.get('pawnpro.server.logEncoding') || 'windows1252').toLowerCase(),
        follow: (cfg.get('pawnpro.server.output.follow') || 'visible'),
    };
}
async function loadSampConfig(cwd) {
...
class LogTailer {
    chan = vscode.window.createOutputChannel('PawnPro Server');
    timer;
    running = false;
    reading = false;
    file = '';
    ...
    append(s) { this.chan.append(s); this.maybeFollow(); }
    appendLine(s) { this.chan.appendLine(s); this.maybeFollow(); }
...
class ServerController {
    ...
    start() {
        if (this.term) {
            if (!this.restarting)
                vscode.window.showInformationMessage('PawnPro: servidor já está em execução.');
            return;
        }
        ...
        try {
            ...
            if (!IS_WINDOWS && logPath)
                this.tailer.start(logPath, logEncoding);
    ...
    onDidCloseTerminal(_) { }
}
export function registerServerControls(context) {
    ...
    const provider = new ServerViewProvider(context, (text) => { void srv.sendLine(text); });
