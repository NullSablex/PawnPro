...
function countReferences(text, fnNames, mainDeclPos) {
    const lines = text.split(/\r?\n/);
    const refs = new Map();
    let inBlock = false;
...
function findReferencesInText(text, uri, fnName, declLine, declCol) {
    const lines = text.split(/\r?\n/);
    const out = [];
    let inBlock = false;
    const rxCall = /\b([A-Za-z_]\w*)\s*\(/g;
    for (let i = 0; i < lines.length; i++) {
        ...
        while ((m = rxCall.exec(stripped.text))) {
            ...
            if (name !== fnName)
                continue;
            const col = m.index + m[0].indexOf(name);
            if (declLine === i && declCol === col)
                continue;
            out.push(new vscode.Location(uri, new vscode.Range(i, col, i, col + fnName.length)));
        }
    }
    return out;
}
const codeLensCache = new Map();
class PawnCodeLensProvider {
    ...
    async provideCodeLenses(document, _token) {
        ...
        const mainDeclPos = new Map();
        for (const func of funcs) {
            const byLine = mainDeclPos.get(func.name) ?? new Map();
            const cols = byLine.get(func.line) ?? new Set();
            cols.add(func.col);
        ...
        if (codeLensCache.size > 20) {
            ...
            if (first)
                codeLensCache.delete(first);
        }
        return lenses;
    }
}
/* ─── Registration ─────────────────────────────────────────────── */
export function registerCodeLens(context, config) {
    const provider = new PawnCodeLensProvider();
    context.subscriptions.push(vscode.languages.registerCodeLensProvider({ language: 'pawn', scheme: 'file' }, provider));
    context.subscriptions.push(vscode.commands.registerCommand('pawnpro.showReferences', async (uri, line, col, fnName) => {
        ...
        await vscode.commands.executeCommand('editor.action.showReferences', uri, new vscode.Position(line, col), locations);
    }));
    context.subscriptions.push(vscode.workspace.onDidChangeTextDocument(e => {
        if (isPawnFile(e.document.fileName))
