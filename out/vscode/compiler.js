...
export function registerCompileCommand(context, config) {
    ...
    const cmd = vscode.commands.registerCommand('pawnpro.compileCurrent', async () => {
        ...
        try {
            ...
            // Run compilation
            const result = await runCompile(compileArgs.exe, compileArgs.args, compileArgs.cwd, cfg.output.encoding);
            channel.append(result.output);
            if (result.exitCode === 0) {
                vscode.window.showInformationMessage('Compilado com sucesso.');
            }
            else {
                vscode.window.showErrorMessage(`Compilacao falhou. code=${result.exitCode ?? 'null'} signal=${result.signal ?? 'none'}`);
            }
        }
        catch (e) {
