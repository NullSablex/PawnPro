...
export const msg = {
    compiler: {
        alreadyCompiling: (file) => localize('compiler.alreadyCompiling', '"{0}" já está sendo compilado.', file),
        compiling: (file) => localize('compiler.compiling', 'Compilando: {0}', file),
        success: (file) => localize('compiler.success', 'Compilação bem-sucedida: {0}', file),
        failed: (file) => localize('compiler.failed', 'Compilação falhou: {0}', file),
        ...
        compilerNotFound: (path) => localize('compiler.compilerNotFound', 'Falha ao iniciar pawncc: {0}', path),
        detecting: () => localize('compiler.detecting', 'Detectando compilador...'),
        detected: (path) => localize('compiler.detected', 'pawncc detectado: {0}', path),
        notDetected: () => localize('compiler.notDetected', 'Compilador não detectado automaticamente.'),
    },
    diagnostics: {
        includeNotFound: (token) => localize('diagnostics.includeNotFound', 'Include não encontrada: "{0}"', token),
        triedExtension: (token) => localize('diagnostics.triedExtension', ' (tentou: {0}.inc)', token),
        relativePath: (dir) => localize('diagnostics.relativePath', '. Caminho relativo a: {0}', dir),
        searchedIn: (paths) => localize('diagnostics.searchedIn', '. Buscado em: {0}', paths),
        noIncludePaths: () => localize('diagnostics.noIncludePaths', '. Nenhum includePaths configurado.'),
        unusedVariable: (name) => localize('diagnostics.unusedVariable', '"{0}" variável declarada mas não utilizada', name),
        unusedStock: (name) => localize('diagnostics.unusedStock', '"{0}" função stock declarada mas não utilizada', name),
    },
    hover: {
        ...
        line: (n) => localize('hover.line', ' linha {0}', n),
    },
    codelens: {
        reference: () => localize('codelens.reference', '1 referência'),
        references: (n) => localize('codelens.references', '{0} referências', n),
    ...
    server: {
        ...
        notFound: (path) => localize('server.notFound', 'Servidor não encontrado: {0}', path),
        ...
        failedStart: (err) => localize('server.failedStart', 'Falha ao iniciar servidor: {0}', err),
        rconFailed: (err) => localize('server.rconFailed', 'Falha ao enviar RCON: {0}', err),
    ...
    general: {
        ...
        cacheStats: (stats) => localize('general.cacheStats', 'Estatísticas do cache: {0}', stats),
    },
    debug: {
...
export function buildIncludeErrorMessage(token, hasExtension, isRelative, fromDir, includePaths) {
