...
export function activateDiagnostics(context, config) {
    ...
    context.subscriptions.push(vscode.workspace.onDidOpenTextDocument(doc => run(doc)), vscode.workspace.onDidSaveTextDocument(doc => run(doc)), vscode.workspace.onDidChangeTextDocument(e => run(e.document)), vscode.workspace.onDidCloseTextDocument(doc => collection.delete(doc.uri)), vscode.workspace.onDidChangeWorkspaceFolders(() => runActive()));
