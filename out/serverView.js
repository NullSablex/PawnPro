export class ServerViewProvider {
    ...
    favorites = [];
    history = [];
    constructor(ctx, onSend) {
        this.ctx = ctx;
        this.onSend = onSend;
        this.favorites = ctx.workspaceState.get('pawnpro.server.favorites', []) ?? [];
        this.history = ctx.workspaceState.get('pawnpro.server.history', []) ?? [];
    }
