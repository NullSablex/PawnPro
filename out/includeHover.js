...
function fileOpenLink(label, fileUri, line) {
...
function prettyPath(resolved) {
    return resolved.replace(/\\/g, '/');
}
function prettyPathFromDoc(resolved, doc) {
...
function posInAnyString(ch, line, spans) {
 ...
 */
function stripCommentsPreserveColumns(line, inBlock) {
    const spans = getStringSpans(line);
    const out = line.split('');
    const mask = new Array(line.length).fill(false);
    const mark = (i) => { out[i] = ' '; mask[i] = true; };
    let i = 0;
    while (i < line.length) {
        if (!inBlock) {
...
/* ===========================
   ...
   =========================== */
function parseLocalFunctionsByLines(doc) {
    const out = [];
...
function parseMacroLocalFunctionsByLines(doc, defs) {
    if (!defs.placeholder.length)
        return [];
    ...
    for (let i = 0; i < doc.lineCount; i++) {
        ...
        if (!m)
            continue;
        if (!bases.has(m[1]))
            continue; // apenas se houver placeholder correspondente
        const name = m[2];
        const params = (m[3] ?? '').trim();
...
function parseDefines(text) {
    ...
    // Alias simples: #define Msg SendClientMessage
    {
        ...
        let m;
        while ((m = rxAlias.exec(clean)))
            alias.push({ name: m[1], target: m[2] });
    }
    // Tipo função: #define Foo(a,b) corpo...
    {
        ...
        while ((m = rxFn.exec(clean))) {
            if (m[1].includes('%0'))
                continue; // placeholders tratados abaixo
    ...
    // Placeholder base como BPR::%0(%1)
    {
        ...
        let m;
        while ((m = rxPH.exec(clean)))
            placeholder.push({ macroBase: m[1], body: m[2] });
    }
    return { alias, fn, placeholder };
}
function expandPlaceholderBody(body, name, args) {
...
/* ================
   Utilitários linha
   ================ */
function rangeInAnyString(range, line, spans) {
    const ss = spans ?? getStringSpans(line);
...
function collectCalls(line) {
    const results = [];
    const spans = getStringSpans(line);
    function pushCall(ident, nameStart, openIdx) {
    ...
    // placeholders Base::Nome(...)
    {
        ...
        while ((m = rxPH.exec(line))) {
            ...
            if (posInAnyString(openIdx, line, spans))
                continue;
...
function innermostCallAt(line, ch) {
...
function wordLooksLikeCall(line, range, spans) {
    const ss = spans ?? getStringSpans(line);
    if (rangeInAnyString(range, line, ss))
        return false;
...
function chooseBestLocals(locals) {
    ...
    for (const f of locals) {
        ...
        if (!cur || rank[f.kind] > rank[cur.kind])
            byName.set(f.name, f);
    }
...
/* ================================
   ...
   ================================ */
function collectIncludeTokens(text) {
...
async function gatherIncludedUris(rootDoc, includePaths, maxDepth = 3, maxFiles = 30) {
    const out = [];
    const seen = new Set();
    const queue = [];
...
async function findInIncludedFiles(ident, curDoc, includePaths) {
...
/* =======================
   ...
   ======================= */
export function registerIncludeHover(context, getIncludePaths) {
    const provider = vscode.languages.registerHoverProvider('pawn', {
        async provideHover(doc, pos) {
            ...
            if (posInAnyString(pos.character, lineText, spans))
                return undefined;
            ...
            for (const ident of tryIdentsInOrder) {
                ...
                if (alias) {
                    ...
                    const targetEntry = await (async () => {
                        ...
                        if (cross) {
                            return { name: alias.target, signature: cross.def.signature, file: cross.uri.fsPath, doc: undefined };
                        }
                ...
                if (fn) {
                    ...
                    if (fn.file)
                        md.appendMarkdown(`${prettyPathFromDoc(fn.file, doc)}\n\n`);
...
//# sourceMappingURL=includeHover.js.map