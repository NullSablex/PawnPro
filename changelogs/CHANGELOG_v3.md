...
- **Títulos de comando traduzidos em espanhol, romeno e russo** — os nomes que aparecem na paleta de comandos (`PawnPro: Compilar arquivo atual`, `PawnPro: Iniciar Servidor`, …) ainda estavam em inglês nesses três idiomas: os arquivos `package.nls.*` traziam os valores ingleses como marcador, com um comentário pedindo tradução. As 22 chaves em uso passam a estar traduzidas nos cinco idiomas, e as 36 chaves que sobraram da época em que a configuração vivia no `settings.json` do editor foram removidas
